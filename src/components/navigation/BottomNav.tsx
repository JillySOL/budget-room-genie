  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around items-center py-2 z-50">
      <Link to={homePath} className="flex flex-col items-center">
        <Home className={`h-6 w-6 ${isActive(homePath) ? 'text-budget-accent' : 'text-gray-500'}`} />
        <span className={`text-xs mt-1 ${isActive(homePath) ? 'text-budget-accent font-medium' : 'text-gray-500'}`}>Home</span>
      </Link>
